...
public class ServerController extends Thread {
  ...
  /**
   * Reads a text file from the files folder and adds them to the contacts array. Then sets the
   * contacts array to oldContactList array.
   *
  ...
  public User checkLoginUser(User user) {
    if (userRegister.getUserHashMap().size() != 0) {
      ...
      if (userRegister.getUserHashMap().containsKey(
          user.getUsername())) { //userRegister.getUserHashMap().get(user.getUserName()).getUserName().equals(user.getUserName())
  ...
  /**
   * Receives a username and gets a User-object from the HashMap. If the user's activity has been
   * delayed a delayed activity is sent to the sendBuffer, else a new activity is sent to the
   * client.
   *
  ...
  /**
   * Receives an activity object and gets a user with the help of the activity object and sets a
   * user's delayed activity with the activity. Then gets the connected userTimer and set the delay
   * to 5min and set the activeDelay to true and then starts the Timer.
   *
  ...
  /**
   * receives a UserTimer object and update the UserTimer's User object and stops the timer and sets
   * the currentTime with the recent currentTime and starts the timer.
   *
